...
class SSP {
    ...
    static function innerJoin($request, $sql_details, $from, $tables, $primaryKey, $columns,$where){
        $bindings = array();
        $db = SSP::sql_connect( $sql_details );
        //$limit = SSP::limit( $request, $columns );
        //$order = SSP::order( $request, $columns, $joinQuery );
        ...
        for ($i=0;$i < sizeof($tables); $i++){
            $table .= "LEFT JOIN ".$tables[$i][name]." ON ".$tables[$i]['col1']." = ".$tables[$i]['col2']." ";
        }
        $query = "SELECT ".implode(", ", SSP::pluck($columns, 'db'))."$table $where";
        $data = SSP::sql_exec( $db, $bindings,$query);

           // Data set length after filtering
        $resFilterLength = SSP::sql_exec( $db, $bindings,
             "SELECT COUNT($primaryKey) from $from $where"
        );
        $recordsFiltered = $resFilterLength[0][0];

             // Total data set length
        $resTotalLength = SSP::sql_exec( $db, $bindings,
            "SELECT COUNT($primaryKey) from $from $whereAllSql"
        );
        $recordsTotal = $resTotalLength[0][0];
