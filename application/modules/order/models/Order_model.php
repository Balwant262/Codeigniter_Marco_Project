	...
	/**
      ...
      */
  	...
  	public function getlastid(){
  		...
  		$query =$this->db->query("SELECT * FROM orders ORDER BY order_id DESC LIMIT 1");
  		$result = $query->row();
		return $query->result();
  	}	
  	...
  	public function getassproductname($id){
  			$query = $this->db->select('*')->from('assemble')->where('assemble_id',$id)->get();
  		$res = $query->row();
  		return $res->part_name;
  	}
  	public function getproductlist($id){
  		$query=$this->db->select('*')->from('inquiry_products')->where('inq_id',$id)->get();
  		return $result=$query->result();
  	}
  	public function getremprodpcs($id,$prod){
  		$query=$this->db->select('*')->from('order_products')->where('batch_no',$id)->where('part_nm',$prod)->get();
  	
  		$res = $query->row();
  		// return $this->db->last_query();
  		return $res->prod_pcs;
  	}
  	public function getremassprodpcs($id,$asprod){
  		$query=$this->db->select('*')->from('order_products')->where('batch_no',$id)->where('assemble_prod',$asprod)->get();
  		$res = $query->row();
  		// return $this->db->last_query();
  		return $res->prod_pcs;
	}
