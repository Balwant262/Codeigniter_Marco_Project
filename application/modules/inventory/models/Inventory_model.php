...
class Inventory_model extends CI_Model {       
	...
	/**
      ...
      */
  	...
  	public function getassemblyname($id){
  		$query=$this->db->select('*')->from('assemble')->where('assemble_id',$id)->get();
  		$result=$query->row();
  		return $result->part_name;
  	}
  	public function getprodhold($id){
  		$query = $this->db->select('*')->from('order_products')->where('part_nm',$id)->get();
  		return $query->result();
  	}
  	public function getassemblyprodhold($id){
  		$query = $this->db->select('*')->from('order_products')->where('assemble_prod',$id)->get();
  		return $query->result();
  	}
}