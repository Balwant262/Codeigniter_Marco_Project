...
class User_model extends CI_Model {       
	...
	{
		$query = $this->db->select('name')->from('users')->where('users_id',$id)->get();
		$res = $query->row();
		$name = $res->name;
		return $name;
	}

	public function check_if_user_active($id)
	{
		$query = $this->db->select('is_deleted')->from('users')->where('users_id',$id)->get();
		$result = $query->row();
		$useract = $result->is_deleted;
		if($useract == '0')
		{
			return true;
		}
